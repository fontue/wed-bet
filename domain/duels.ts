...
export const DUEL_GAME_META: Record<
  DuelGame,
  { title: string; description: string; emoji: string }
> = {
  HIGH_CARD: {
    title: "Старшая карта",
    description: "Туз старше короля. При ничьей колода сдаёт заново.",
    emoji: "🂡",
  },
  DICE: {
    title: "Кости фортуны",
    description: "По две кости каждому. Побеждает большая сумма.",
    emoji: "🎲",
  },
  SLOTS: {
    title: "Свадебный слот",
    description: "Тройка старше пары, пара старше высокого символа.",
    emoji: "🎰",
  },
};

export const SLOT_SYMBOL_META: Record<
  SlotSymbol,
...
function slotScore(slots: [SlotSymbol, SlotSymbol, SlotSymbol]): {
  score: number[];
  combination: SlotDuelResult["challengerCombination"];
} {
  const ranks = slots
    .map((symbol) => SLOT_SYMBOL_META[symbol].rank)
    .sort((a, b) => b - a);
  ...
  const combination =
    groups[0][1] === 3 ? "THREE" : groups[0][1] === 2 ? "PAIR" : "HIGH";
  const category = combination === "THREE" ? 3 : combination === "PAIR" ? 2 : 1;
  return {
    score: [
      category,
      ...groups.flatMap(([rank, count]) => Array(count).fill(rank)),
    ],
    combination,
  };
}
...
export function generateDuelResult(
  game: DuelGame,
  randomInt: DuelRandomInt,
): { result: DuelResult; winner: "CHALLENGER" | "OPPONENT" } {
  let redraws = 0;
  while (redraws < 128) {
    if (game === "HIGH_CARD") {
      ...
      if (challengerCard.rank !== opponentCard.rank) {
        const result: HighCardDuelResult = {
          game,
          challengerCard,
          opponentCard,
          redraws,
        ...
        return {
          result,
          winner:
            challengerCard.rank > opponentCard.rank ? "CHALLENGER" : "OPPONENT",
        };
      }
    }
    if (game === "DICE") {
    ...
    if (game === "SLOTS") {
      ...
      if (comparison !== 0) {
        const result: SlotDuelResult = {
          game,
          challengerSlots,
          opponentSlots,
          challengerCombination: challenger.combination,
          opponentCombination: opponent.combination,
          redraws,
        };
...
export function cardRankLabel(rank: number): string {
  ...
  return (
    ({ 11: "В", 12: "Д", 13: "К", 14: "Т" } as Record<number, string>)[rank] ??
    String(rank)
  );
}

export function cardSuitLabel(suit: PlayingCard["suit"]): string {
  return ({ HEARTS: "♥", DIAMONDS: "♦", CLUBS: "♣", SPADES: "♠" } as const)[
    suit
  ];
}
