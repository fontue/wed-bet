...
export const DogHouseReel=memo(function DogHouseReel({reel,cells,spinning,stopping,speed,reducedMotion,roundSeed,spinIndex,isFreeSpin}:{reel:number;cells:DogHouseDisplayCell[];spinning:boolean;stopping:boolean;speed:DogHouseSpeed;reducedMotion:boolean;roundSeed:string;spinIndex:number;isFreeSpin:boolean}){
  ...
  return <div className={`dogslot-reel ${stopping?"is-stopping":""}`} data-reel={reel}><div className={`dogslot-reel-result ${spinning?"is-spinning":"is-landed"}`}>{cells.map((cell)=><div className={`dogslot-cell ${cell.isWinning?"is-winning":""} ${cell.isDimmed?"is-dimmed":""} ${cell.isSticky?"is-sticky":""} ${cell.isNewSticky?"is-new-sticky":""} ${cell.isBonus?"is-bonus":""}`} key={cell.animationId} data-animation-id={cell.animationId} data-cell-index={cell.row*5+cell.reel} role="gridcell"><DogHouseSymbolIcon cell={{id:Number(cell.roundCellId),symbol:cell.symbol,multiplier:cell.multiplier}}/></div>)}</div>{spinning&&<div className={`dogslot-target-strip ${stopping?"is-stopping":""} ${reducedMotion?"is-reduced":""}`} style={style} aria-hidden="true"><div>{track.map((cell,index)=><span key={`${cell.id}-${index}`}><DogHouseSymbolIcon cell={cell}/></span>)}</div></div>}</div>;
});
