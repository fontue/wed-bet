...
export function DogHouseGrid({
  ...
  return (
    <div
      className="dogslot-reels dogslot-reels-v2"
      role="grid"
      aria-label="Пять барабанов, три ряда"
    >
      {reels.map((cells, reel) => (
        <DogHouseReel
          key={reel}
          reel={reel}
          cells={cells}
          spinning={reel >= stoppedReels}
          stopping={reel === stoppingReel}
          speed={speed}
          fastForward={fastForward}
          reducedMotion={reducedMotion}
          roundSeed={model.roundId}
          spinIndex={model.spinIndex}
          isFreeSpin={model.isFreeSpin}
        />
      ))}
      <PaylineOverlay line={line} />
    </div>
  );
}
