...
const round = (seed: string): DogHouseSlotRound => {
  ...
  return {
    ...
    balanceAfter: 99_900 + result.totalPayout,
...
const reveal = (value: 1 | 2 | 3) => {
  const item = structuredClone(round("patch-342")),
    total = value * 9,
    source = item.result.freeSpins;
  item.id += `-reveal-${total}`;
  item.result.freeSpinReveal = Array(9).fill(value);
  item.result.awardedFreeSpins = total;
  item.result.freeSpins = Array.from({ length: total }, (_, index) =>
    structuredClone(source[index % source.length]),
  );
  item.result.bonusPayout = item.result.freeSpins.reduce(
    (sum, spin) => sum + spin.payout,
    0,
  );
  item.bonusWin = item.result.bonusPayout;
  item.result.totalPayout =
    item.result.baseGamePayout +
    item.result.bonusScatterPayout +
    item.result.bonusPayout;
  item.totalWin = item.result.totalPayout;
  item.balanceAfter = item.balanceBefore - item.chargedAmount + item.totalWin;
  return item;
};
...
export const DOG_HOUSE_SCENARIOS = [
  ...
  {
    id: "anticipation-miss",
    title: "Anticipation miss",
    round: round("patch-26"),
  },
  {
    id: "anticipation-hit",
    title: "Anticipation hit",
    round: round("patch-342"),
  },
  ...
  {
    id: "sticky-many",
    title: "Multiple Sticky Wild",
    round: round("patch-428"),
  },
