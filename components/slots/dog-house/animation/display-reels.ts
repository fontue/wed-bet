...
export interface DogHouseDisplayModel{cells:DogHouseDisplayCell[];stickyIds:Map<number,string>;roundId:string;spinIndex:number;isFreeSpin:boolean}
export const dogHouseIndex=(reel:number,row:number)=>row*5+reel;

export function createDisplayModel(spin:DogHouseSpin,{roundId,spinIndex,isFreeSpin=false,previousSticky=new Map<number,string>(),winning=new Set<number>(),dimmed=false}:{roundId:string;spinIndex:number;isFreeSpin?:boolean;previousSticky?:Map<number,string>;winning?:Set<number>;dimmed?:boolean}):DogHouseDisplayModel{
  ...
  return{cells,stickyIds,roundId,spinIndex,isFreeSpin};
}

export function withWinningLine(model:DogHouseDisplayModel,positions:number[]):DogHouseDisplayModel{const winning=new Set(positions);return{...model,cells:model.cells.map((cell,index)=>({...cell,isWinning:winning.has(index),isDimmed:!winning.has(index)}))};}
export function clearWinningLine(model:DogHouseDisplayModel):DogHouseDisplayModel{return{...model,cells:model.cells.map((cell)=>({...cell,isWinning:false,isDimmed:false,isNewSticky:false}))};}
