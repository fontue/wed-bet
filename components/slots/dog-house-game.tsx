...
export function DogHouseGame({initialState}:{initialState:InitialState}){
  ...
  async function spin(){
    ...
    try{const response=await fetch("/api/slots/casa-degli-sposi/spin",{method:"POST",cache:"no-store",headers:{"content-type":"application/json"},body:JSON.stringify({gameId:"casa-degli-sposi",stake:request.stake,idempotencyKey:request.idempotencyKey})}),body=await response.json() as {round?:DogHouseSlotRound;error?:string};if(!response.ok||!body.round){if(request.debited)setBalance((value)=>value+request.stake);pending.current=undefined;throw new Error(body.error??"Spin не выполнен");}setBalance(body.round.balanceBefore-body.round.chargedAmount);await playback.start(body.round);}catch(cause){playback.reset();if(pending.current)setError("Связь оборвалась. Повторите Spin — будет использован тот же ключ без нового списания.");else setError(cause instanceof Error?cause.message:"Spin не выполнен");}
  }
