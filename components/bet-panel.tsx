...
export function BetPanel({ initialEvent, initialBalance, bonuses }: { initialEvent: MarketEvent; initialBalance: number; bonuses: UserBonus[] }) {
  ...
  async function submit() {
    ...
    const payload = { eventId: event.id, outcomeId, amount, bonusId, idempotencyKey: createClientRequestId(), queuedAt: new Date().toISOString() };
    if (!navigator.onLine) {
    ...
    try {
      ...
      setMessage(`Ставка принята сервером. Коэффициент сейчас X${formatCoefficient(body.bet.projectedCoefficient)}.`);
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_23rem]">
      ...
      <aside className="card h-fit p-5 lg:sticky lg:top-24">
        ...
        {quote && selected && <div className="mt-5 rounded-2xl bg-[#174b38] p-4 text-white"><div className="flex items-end justify-between"><span className="text-xs text-white/65">Коэффициент этой ставки</span><strong className="text-3xl">X{formatCoefficient(quote.coefficient)}</strong></div><div className="mt-3 flex justify-between border-t border-white/15 pt-3 text-sm"><span className="text-white/65">Если закроется сейчас</span><strong>{formatLira(quote.projectedPayout)}</strong></div></div>}
