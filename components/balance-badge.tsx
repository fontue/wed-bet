...
export function BalanceBadge({ initialBalance }: { initialBalance: number }) {
  ...
  return (
    <Link
      href="/profile"
      className="rounded-full bg-white px-3.5 py-2 text-sm font-extrabold text-[#174b38] shadow-sm"
    >
      <span className="mr-1.5 text-[#c89537]">₤</span>
      {formatLira(balance).replace(" ₤", "")}
    </Link>
  );
}
