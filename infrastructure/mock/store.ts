...
function initialState(): MockState {
  return {
    ...
    userBonuses: [
      {
        id: "ub-welcome",
        userId: "u-sofia",
        definitionId: "bonus-back",
        label: "Возврат 30%",
        kind: "CASHBACK",
        value: 30,
        status: "AVAILABLE",
        expiresAt: isoAfter(95 * 60_000),
    ...
    slotSettings: {
      ...
      enabled: true,
      spinsEnabled: true,
      allowedBets: [...LEMONZA_BETS],
      minBet: LEMONZA_BETS[0],
      maxBet: LEMONZA_BETS.at(-1)!,
      lemonBoostEnabled: true,
    ...
    dogHouseSettings: {
      ...
      enabled: true,
      spinsEnabled: true,
      allowedBets: [...DOG_HOUSE_BETS],
      minBet: DOG_HOUSE_BETS[0],
      maxBet: DOG_HOUSE_BETS.at(-1)!,
    },
    loginTokens:
        ...
        : [
            {
              id: "lt-guest",
              userId: "u-sofia",
              tokenHash: hash("guest-demo"),
              expiresAt: isoAfter(365 * 24 * 60 * 60 * 1000),
            },
            {
              id: "lt-misha",
              userId: "u-misha",
              tokenHash: hash("misha-demo"),
              expiresAt: isoAfter(365 * 24 * 60 * 60 * 1000),
            },
            {
              id: "lt-admin",
              userId: "u-admin",
              tokenHash: hash("admin-demo"),
              expiresAt: isoAfter(365 * 24 * 60 * 60 * 1000),
            },
          ],
    sessions: [],
...
export const store =
  globalStore.__wedBetMockStore?.schemaVersion === MOCK_STATE_VERSION
...
if (
  store.slotSettings.allowedBets.some(
    (value) => !LEMONZA_BETS.includes(value as (typeof LEMONZA_BETS)[number]),
  )
) {
...
export function createEvent(input: {
  ...
  closesAt: string;
  liquidity: number;
  outcomes: Array<{ title: string; probability: number }>;
}): MarketEvent {
  if (
    !input.title?.trim() ||
  ...
  if (
    ...
    input.outcomes.some((item) => !item.title.trim())
  )
    throw new Error("INVALID_OUTCOMES");
  const totalProbability = input.outcomes.reduce(
    (sum, item) => sum + Number(item.probability),
    0,
  );
  if (Math.abs(totalProbability - 100) > 0.001)
    throw new Error("INVALID_PROBABILITY");
  if (!Number.isInteger(input.liquidity) || input.liquidity <= 0)
    throw new Error("INVALID_LIQUIDITY");
  if (!input.closesAt || new Date(input.closesAt).getTime() <= Date.now())
    throw new Error("INVALID_CLOSE_TIME");
  const id = randomUUID();
  const rawSeeds = input.outcomes.map(
    (item) => (input.liquidity * item.probability) / 100,
  );
  const seeds = rawSeeds.map(Math.floor);
  const remainder =
    input.liquidity - seeds.reduce((sum, value) => sum + value, 0);
  const order = rawSeeds
    .map((value, index) => ({ index, fraction: value % 1 }))
    .sort((a, b) => b.fraction - a.fraction);
  for (let index = 0; index < remainder; index += 1)
    seeds[order[index % order.length].index] += 1;
  const event: MarketEvent = {
    ...
    createdAt: new Date().toISOString(),
    outcomes: input.outcomes.map((item, index) => ({
      id: randomUUID(),
      title: item.title.trim(),
      order: index,
      initialProbabilityBps: Math.round(item.probability * 100),
      seedPool: seeds[index],
      pool: seeds[index],
      bettorsCount: 0,
    })),
  };
...
function refundPendingDuel(
  ...
  store.transactions.push({
    ...
    amount: duel.stake,
    type: "DUEL_REFUND",
    reason: `${duelReason(duel)} · возврат`,
    balanceAfter: challenger.balance,
    operationKey: `duel:${duel.id}:refund`,
    createdAt: at,
  });
  duel.status = status;
...
function expirePendingDuels(): void {
  ...
  for (const duel of store.duels) {
    if (duel.status === "PENDING" && new Date(duel.expiresAt).getTime() <= now)
      refundPendingDuel(duel, "EXPIRED", at);
  }
}
...
export function getDuelState(userId: string) {
  expirePendingDuels();
  const involved = store.duels.filter(
    (duel) => duel.challengerId === userId || duel.opponentId === userId,
  );
  const resolved = involved.filter((duel) => duel.status === "RESOLVED");
  const duelTransactions = store.transactions.filter(
    (transaction) =>
      transaction.userId === userId &&
      ["DUEL_STAKE", "DUEL_WIN", "DUEL_REFUND"].includes(transaction.type),
  );
  return {
    incoming: clone(
      involved
        .filter(
          (duel) => duel.status === "PENDING" && duel.opponentId === userId,
        )
    ...
    outgoing: clone(
      involved
        .filter(
          (duel) => duel.status === "PENDING" && duel.challengerId === userId,
        )
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt)),
    ),
    recent: clone(
      involved
        .filter((duel) => duel.status !== "PENDING")
        .sort((a, b) =>
    ...
    stats: {
      ...
      profit: duelTransactions.reduce(
...
export function createDuel(input: {
  challengerId: string;
  opponentId: string;
  game: DuelGame;
  idempotencyKey: string;
}): { duel: Duel; balance: number } {
  expirePendingDuels();
  ...
  if (existingId) {
    ...
    if (!existing || existing.challengerId !== input.challengerId)
      throw new Error("IDEMPOTENCY_CONFLICT");
    return {
      duel: clone(existing),
      balance: getUser(input.challengerId)!.balance,
    };
  }
  ...
  if (!(["HIGH_CARD", "DICE", "SLOTS"] as DuelGame[]).includes(input.game))
    throw new Error("INVALID_DUEL_GAME");
  if (challenger.balance < DUEL_STAKE) throw new Error("INSUFFICIENT_BALANCE");
  if (
    store.duels.filter(
      (duel) =>
        duel.challengerId === challenger.id && duel.status === "PENDING",
    ).length >= 3
  )
    throw new Error("TOO_MANY_OUTGOING_DUELS");
  if (
    store.duels.some(
      (duel) =>
        duel.status === "PENDING" &&
        [duel.challengerId, duel.opponentId].includes(challenger.id) &&
        [duel.challengerId, duel.opponentId].includes(opponent.id),
    )
  )
    throw new Error("PAIR_DUEL_EXISTS");
  ...
  store.transactions.push({
    ...
    amount: -DUEL_STAKE,
    type: "DUEL_STAKE",
    reason: duelReason(duel),
    balanceAfter: challenger.balance,
    operationKey: `duel:${duel.id}:challenger-stake`,
    createdAt,
  });
...
export function acceptDuel(input: {
  duelId: string;
  opponentId: string;
  idempotencyKey: string;
}): { duel: Duel; balance: number } {
  ...
  if (!duel || duel.opponentId !== input.opponentId)
    throw new Error("DUEL_NOT_FOUND");
  if (duel.status === "RESOLVED")
    return { duel: clone(duel), balance: getUser(input.opponentId)!.balance };
  ...
  if (previousId) {
    ...
    return {
      duel: clone(previous),
      balance: getUser(input.opponentId)!.balance,
    };
  }
  if (duel.status !== "PENDING") throw new Error("DUEL_NOT_PENDING");
  const opponent = store.users.find(
    (user) =>
      user.id === input.opponentId &&
      user.role === "USER" &&
      user.status === "ACTIVE",
  );
  ...
  const generated = generateDuelResult(duel.game, (maxExclusive) =>
    randomInt(maxExclusive),
  );
  const winnerId =
    generated.winner === "CHALLENGER" ? duel.challengerId : duel.opponentId;
  ...
  store.transactions.push({
    ...
    operationKey: `duel:${duel.id}:opponent-stake`,
  ...
  store.transactions.push({
    ...
    amount: duel.pot,
    type: "DUEL_WIN",
    reason: `${duelReason(duel)} · победа`,
    balanceAfter: winner.balance,
    operationKey: `duel:${duel.id}:win`,
    createdAt: acceptedAt,
  });
...
export function declineDuel(input: {
  duelId: string;
  opponentId: string;
  idempotencyKey: string;
}): Duel {
  expirePendingDuels();
  const duel = store.duels.find(
    (item) => item.id === input.duelId && item.opponentId === input.opponentId,
  );
...
export function cancelDuel(input: {
  duelId: string;
  actorId: string;
  idempotencyKey: string;
  admin?: boolean;
  ...
  expirePendingDuels();
  const duel = store.duels.find((item) => item.id === input.duelId);
  if (!duel || (!input.admin && duel.challengerId !== input.actorId))
    throw new Error("DUEL_NOT_FOUND");
...
export function listAllDuels(): Duel[] {
  expirePendingDuels();
  return clone(
    [...store.duels].sort((a, b) => b.createdAt.localeCompare(a.createdAt)),
  );
}
...
export function exchangeLoginToken(rawToken: string): {
  session: Session;
  user: User;
} {
  const token = store.loginTokens.find(
...
export function createDevelopmentSession(rawToken: string): {
  session: Session;
  user: User;
} {
...
export function issueLoginToken(userId: string): string {
  ...
  for (const token of store.loginTokens.filter(
    (item) => item.userId === userId && !item.usedAt,
  ))
    token.revokedAt = new Date().toISOString();
  const rawToken =
    randomUUID().replaceAll("-", "") + randomUUID().replaceAll("-", "");
  store.loginTokens.push({
    id: randomUUID(),
    userId,
    tokenHash: hash(rawToken),
    expiresAt: isoAfter(7 * 24 * 60 * 60 * 1000),
  });
  return rawToken;
}
...
export function createUser(input: {
  ...
  if (!input.displayName.trim() || !input.loginCode.trim())
    throw new Error("INVALID_USER");
  if (
    store.users.some(
      (user) => user.loginCode.toLowerCase() === input.loginCode.toLowerCase(),
    )
  )
  ...
  if (input.balance > 0)
    store.transactions.push({
      ...
      reason: "Стартовый баланс гостя",
      balanceAfter: input.balance,
      operationKey: `user:${user.id}:initial`,
...
export function updateUserAdmin(
  userId: string,
  input: {
    action: "block" | "unblock" | "reset-sessions" | "adjust";
    amount?: number;
    reason?: string;
  },
): User {
  ...
  if (input.action === "reset-sessions")
    for (const session of store.sessions.filter(
      (item) => item.userId === userId,
    ))
      session.revokedAt = new Date().toISOString();
...
export function listUserBonuses(userId: string): UserBonus[] {
  ...
  for (const bonus of store.userBonuses) {
    if (
      ...
      new Date(bonus.expiresAt).getTime() <= now
    )
...
export function placeBet(input: {
  ...
  if (
    !event ||
    event.status !== "OPEN" ||
    new Date(event.closesAt).getTime() <= Date.now()
  )
    throw new Error("EVENT_CLOSED");
  if (!Number.isInteger(input.amount) || input.amount <= 0)
    throw new Error("INVALID_AMOUNT");

  const bonus = input.bonusId
    ? store.userBonuses.find(
        (item) => item.id === input.bonusId && item.userId === user.id,
      )
    : undefined;
  if (
    bonus &&
    ...
    throw new Error("BONUS_UNAVAILABLE");
  const isFreeBet = bonus?.kind === "FREE_BET";
  if (isFreeBet && input.amount > bonus.value)
    throw new Error("FREE_BET_LIMIT");
  if (!isFreeBet && user.balance < input.amount)
  ...
  const isNewBettor = !store.bets.some(
    (bet) =>
  ...
  const bet: Bet = {
    ...
    amount: input.amount,
    status: "ACCEPTED",
    projectedCoefficient: quote.coefficient,
    probabilityAfterBps: quote.probabilityBps,
    bonusId: bonus?.id,
...
export function getBonusState(userId: string) {
  const latest = [...store.spins]
    .filter((spin) => spin.userId === userId)
...
export function spinWheel(userId: string, idempotencyKey: string) {
  ...
  const latest = [...store.spins]
    .filter((spin) => spin.userId === userId)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
  if (latest && new Date(latest.nextSpinAt).getTime() > Date.now())
    throw new Error("SPIN_COOLDOWN");

  const definitions = store.bonusDefinitions.filter(
    (item) => item.active && item.weight > 0,
  );
  const totalWeight = definitions.reduce((sum, item) => sum + item.weight, 0);
  let draw = randomInt(totalWeight);
  const reward =
    definitions.find((item) => (draw -= item.weight) < 0) ??
    definitions.at(-1)!;
  const createdAt = new Date().toISOString();
  const spin: BonusSpin = {
    id: randomUUID(),
    userId,
    definitionId: reward.id,
    idempotencyKey,
    createdAt,
    nextSpinAt: isoAfter(store.spinIntervalMinutes * 60_000),
  };
  ...
  if (reward.kind === "LIRA") {
    ...
    store.transactions.push({
      ...
      amount: reward.value,
      type: "BONUS",
      reason: `Колесо: ${reward.label}`,
      balanceAfter: user.balance,
      operationKey: `spin:${spin.id}`,
      createdAt,
    });
  } else if (reward.kind !== "NONE") {
    store.userBonuses.push({
      id: randomUUID(),
      userId,
      definitionId: reward.id,
      label: reward.label,
      ...
      value: reward.value,
      status: "AVAILABLE",
      expiresAt: isoAfter(reward.validityMinutes * 60_000),
      createdAt,
    });
  }
...
export function resolveEvent(
  ...
  if (event.status === "RESOLVED") {
    if (event.winningOutcomeId !== winningOutcomeId)
      throw new Error("ALREADY_RESOLVED");
    return clone(event);
  }
  const winner = event.outcomes.find(
  ...
  for (const bet of store.bets.filter(
    (item) => item.eventId === event.id && item.status === "ACCEPTED",
  )) {
    bet.settledAt = settledAt;
    if (bet.outcomeId !== winningOutcomeId) {
      bet.status = "LOST";
      bet.payout = 0;
      const losingBonus = bet.bonusId
        ? store.userBonuses.find((item) => item.id === bet.bonusId)
        : undefined;
      if (losingBonus?.kind === "CASHBACK") {
        ...
        store.transactions.push({
          ...
          amount: cashback,
          type: "CASHBACK",
          reason: `Возврат ${losingBonus.value}%: ${event.title}`,
          balanceAfter: user.balance,
          operationKey: `cashback:${event.id}:${bet.id}`,
          createdAt: settledAt,
        });
      }
      continue;
    ...
    store.transactions.push({
      ...
      reason: event.title,
      balanceAfter: user.balance,
      operationKey: `settle:${event.id}:${bet.id}`,
      createdAt: settledAt,
    });
  }
...
export function cancelEvent(eventId: string): MarketEvent {
  ...
  for (const bet of store.bets.filter(
    ...
    if (bonus) {
      bonus.status = "AVAILABLE";
      bonus.usedAt = undefined;
      bonus.expiresAt = isoAfter(180 * 60_000);
    }
    if (bonus?.kind !== "FREE_BET") {
      ...
      store.transactions.push({
        ...
        amount: bet.amount,
        type: "REFUND",
        reason: `Отмена: ${event.title}`,
        balanceAfter: user.balance,
        operationKey: `cancel:${event.id}:${bet.id}`,
        createdAt: now,
      });
    }
  }
...
export function getLeaderboard(): Array<
  User & { profit: number; wins: number }
> {
  return store.users
    ...
    .map((user) => {
      ...
      const profit = transactions
        .filter(
          (item) =>
            item.type !== "INITIAL_BALANCE" && item.type !== "ADMIN_ADJUSTMENT",
        )
        .reduce((sum, item) => sum + item.amount, 0);
      const wins = store.bets.filter(
        (bet) => bet.userId === user.id && bet.status === "WON",
      ).length;
...
export function getNews() {
  return clone(store.news);
}
export function getMarketView(event: MarketEvent) {
  return marketView(event);
}

export function getBonusDefinitions() {
  return {
    definitions: clone(store.bonusDefinitions),
    intervalMinutes: store.spinIntervalMinutes,
  };
}
export function updateBonusSettings(input: {
  intervalMinutes: number;
  definitions: Array<{ id: string; weight: number; active: boolean }>;
}) {
  if (!Number.isInteger(input.intervalMinutes) || input.intervalMinutes < 1)
    throw new Error("INVALID_INTERVAL");
  if (!input.definitions.some((item) => item.active && item.weight > 0))
    throw new Error("EMPTY_WHEEL");
  store.spinIntervalMinutes = input.intervalMinutes;
  for (const update of input.definitions) {
    const definition = store.bonusDefinitions.find(
      (item) => item.id === update.id,
    );
    if (definition) {
      definition.weight = Math.max(0, Math.floor(update.weight));
      definition.active = Boolean(update.active);
    }
  }
    ...
    history: clone(
      store.slotRounds
        .filter(
          (
            round,
          ): round is Extract<AnySlotRound, { gameId: "sweet-lemonza" }> =>
            round.gameId === "sweet-lemonza" && round.userId === userId,
        )
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
        .slice(0, 20),
    ),
...
export function spinSweetLemonza(input: {
  userId: string;
  stake: number;
  mode: LemonzaMode;
  idempotencyKey: string;
}): { round: SlotRound; balance: number } {
  const existingRoundId = store.slotSpinRequests.get(input.idempotencyKey);
  if (existingRoundId) {
    const existing = store.slotRounds.find(
      (round) => round.id === existingRoundId,
    );
    if (
      !existing ||
      existing.gameId !== "sweet-lemonza" ||
  ...
  if (!store.slotSettings.enabled || !store.slotSettings.spinsEnabled)
    throw new Error("SLOT_DISABLED");
  if (
    !(["STANDARD", "LEMON_BOOST", "BONUS_BUY"] as LemonzaMode[]).includes(
      input.mode,
    )
  )
    throw new Error("INVALID_MODE");
  if (input.mode === "LEMON_BOOST" && !store.slotSettings.lemonBoostEnabled)
    throw new Error("MODE_DISABLED");
  if (input.mode === "BONUS_BUY" && !store.slotSettings.bonusBuyEnabled)
    throw new Error("MODE_DISABLED");
  if (
    !Number.isInteger(input.stake) ||
    !store.slotSettings.allowedBets.includes(input.stake) ||
    input.stake < store.slotSettings.minBet ||
    input.stake > store.slotSettings.maxBet
  )
  ...
  try {
    const result = runSweetLemonzaRound(input.stake, createCryptoRng(), {
      captureDetails: true,
      includeBonus: true,
      mode: input.mode,
    });
    if (user.balance < result.chargedAmount)
      throw new Error("INSUFFICIENT_BALANCE");
    const createdAt = new Date().toISOString();
    ...
    store.transactions.push({
      ...
      type: "SLOT_BET",
      reason: `Sweet Lemonza · ${input.mode}`,
      balanceAfter: user.balance,
      operationKey: `slot:${input.idempotencyKey}:bet`,
      createdAt,
    });
    if (result.totalPayout > 0) {
      ...
      store.transactions.push({
        ...
        type: "SLOT_WIN",
        reason: `Sweet Lemonza · выигрыш ${result.totalPayout}`,
        balanceAfter: user.balance,
        operationKey: `slot:${input.idempotencyKey}:win`,
        createdAt,
      });
    }
    const round: SlotRound = {
      ...
      scatterWin: result.scatterPayout,
    ...
    const userRounds = store.slotRounds
      .filter(
        (item) => item.gameId === "sweet-lemonza" && item.userId === user.id,
      )
      .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    if (userRounds.length > 100) {
      const removeIds = new Set(userRounds.slice(100).map((item) => item.id));
      store.slotRounds = store.slotRounds.filter(
...
export function getDogHouseState(userId: string) {
  ...
  return {
    userId,
    game: DOG_HOUSE_GAME,
    settings: clone(store.dogHouseSettings),
    balance: user.balance,
    history: clone(
      store.slotRounds
        .filter(
          (
            round,
          ): round is Extract<AnySlotRound, { gameId: "casa-degli-sposi" }> =>
            round.gameId === "casa-degli-sposi" && round.userId === userId,
        )
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
        .slice(0, 20),
    ),
...
export function spinDogHouse(input: {
  userId: string;
  stake: number;
  idempotencyKey: string;
}) {
  ...
  if (existingRoundId) {
    ...
    if (
      !existing ||
      existing.gameId !== "casa-degli-sposi" ||
      existing.userId !== input.userId
    )
      throw new Error("IDEMPOTENCY_CONFLICT");
    return { round: clone(existing), balance: existing.balanceAfter };
  ...
  if (!user) throw new Error("USER_UNAVAILABLE");
  if (!store.dogHouseSettings.enabled || !store.dogHouseSettings.spinsEnabled)
    throw new Error("SLOT_DISABLED");
  if (
    !Number.isInteger(input.stake) ||
    !store.dogHouseSettings.allowedBets.includes(input.stake)
  )
    throw new Error("INVALID_STAKE");
  if (store.slotInFlightUsers.has(user.id)) throw new Error("SPIN_IN_PROGRESS");
  store.slotInFlightUsers.add(user.id);
  try {
    ...
    const result = runDogHouseRound(input.stake, createCryptoRng()),
      createdAt = new Date().toISOString(),
    ...
    if (result.totalPayout > 0) {
      ...
      store.transactions.push({
        ...
        amount: result.totalPayout,
        type: "SLOT_WIN",
        reason: `Casa degli Sposi · выигрыш ${result.totalPayout}`,
        balanceAfter: user.balance,
        operationKey: `slot:${input.idempotencyKey}:win`,
        createdAt,
      });
    }
    const round: Extract<AnySlotRound, { gameId: "casa-degli-sposi" }> = {
      id: randomUUID(),
      gameId: "casa-degli-sposi",
      mathVersion: result.mathVersion,
      userId: user.id,
    ...
    const userRounds = store.slotRounds
      .filter(
        (item) => item.gameId === "casa-degli-sposi" && item.userId === user.id,
      )
      .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    if (userRounds.length > 100) {
      const removeIds = new Set(userRounds.slice(100).map((item) => item.id));
...
export function getSweetLemonzaAdminState(query?: string) {
  const rounds = store.slotRounds
    .filter(
      (round): round is Extract<AnySlotRound, { gameId: "sweet-lemonza" }> =>
        round.gameId === "sweet-lemonza",
    )
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  const filtered = query
    ? rounds.filter((round) =>
        round.id.toLowerCase().includes(query.toLowerCase()),
      )
    : rounds;
  const totalStake = rounds.reduce(
    (sum, round) => sum + (round.chargedAmount ?? round.stake),
    0,
  );
  ...
  return {
    ...
    analytics: {
      ...
      bonusFrequency:
        rounds.length > 0
          ? rounds.filter((round) => round.bonusTriggered).length /
            rounds.length
          : 0,
      biggestWin: rounds.reduce(
        (max, round) => Math.max(max, round.totalWin),
        0,
      ),
    },
  };
...
export function updateSweetLemonzaSettings(input: {
  enabled: boolean;
  ...
  const allowedBets = [...new Set(input.allowedBets)]
  ...
  if (
    ...
    throw new Error("INVALID_BETS");
  store.slotSettings = {
    gameId: "sweet-lemonza",
    enabled: Boolean(input.enabled),
    spinsEnabled: Boolean(input.spinsEnabled),
    allowedBets,
    minBet: allowedBets[0],
    maxBet: allowedBets.at(-1)!,
    lemonBoostEnabled: Boolean(input.lemonBoostEnabled),
    bonusBuyEnabled: Boolean(input.bonusBuyEnabled),
  };
  return clone(store.slotSettings);
}

export function getDogHouseAdminState(query?: string) {
  const rounds = store.slotRounds
      .filter(
        (
          round,
        ): round is Extract<AnySlotRound, { gameId: "casa-degli-sposi" }> =>
          round.gameId === "casa-degli-sposi",
      )
      .sort((a, b) => b.createdAt.localeCompare(a.createdAt)),
    filtered = query
      ? rounds.filter((round) =>
      ...
      : rounds,
    totalStake = rounds.reduce((sum, round) => sum + round.chargedAmount, 0),
    totalWin = rounds.reduce((sum, round) => sum + round.totalWin, 0);
  return {
    settings: clone(store.dogHouseSettings),
    rounds: clone(filtered.slice(0, 100)),
    analytics: {
      ...
      totalStake,
      totalWin,
      actualRtp: totalStake ? totalWin / totalStake : 0,
      bonusFrequency: rounds.length
        ? rounds.filter((round) => round.bonusTriggered).length / rounds.length
        : 0,
      biggestWin: rounds.reduce(
        (max, round) => Math.max(max, round.totalWin),
        0,
...
export function updateDogHouseSettings(input: {
  enabled: boolean;
  spinsEnabled: boolean;
  allowedBets: number[];
}): DogHouseOperationalSettings {
  ...
  if (
    ...
    allowedBets.some(
      (value) =>
        !DOG_HOUSE_BETS.includes(value as (typeof DOG_HOUSE_BETS)[number]),
    )
  )
    throw new Error("INVALID_BETS");
  store.dogHouseSettings = {
  ...
  return clone(store.dogHouseSettings);
}
