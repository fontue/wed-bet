...
import { LEMONZA_BASE_GAME_SYMBOL_WEIGHTS, LEMONZA_BONUS_BUY_MULTIPLIER_CHANCE_BPS, LEMONZA_BONUS_BUY_SYMBOL_WEIGHTS, LEMONZA_BOOST_FREE_SPINS_MULTIPLIER_CHANCE_BPS, LEMONZA_BOOST_SYMBOL_WEIGHTS, LEMONZA_FREE_SPINS_MULTIPLIER_CHANCE_BPS, LEMONZA_FREE_SPINS_SYMBOL_WEIGHTS, LEMONZA_MATH_VERSION, LEMONZA_MULTIPLIER_WEIGHTS, LEMONZA_SYMBOLS } from "../domain/slots/sweet-lemonza/config";
...
export function simulateSweetLemonza(options: CliOptions) {
  ...
  let totalCharged=0,totalWin=0,baseWin=0,scatterWin=0,bonusWin=0,hits=0,bonuses=0,cascades=0,maxCascades=0,maxWin=0,bonusRoundsWin=0,mean=0,m2=0;
  for (let index=0; index<options.spins; index+=1) {
    ...
    if(result.totalPayout>0)hits+=1; if(result.bonusTriggered){bonuses+=1;bonusRoundsWin+=result.scatterPayout+result.bonusPayout;}
  ...
  return { mathVersion:LEMONZA_MATH_VERSION,mode:options.mode,seed:options.seed,paidSpins:options.spins,stake:options.stake,totalStake:totalCharged,totalWin,rtpPercent:totalWin/totalCharged*100,hitFrequencyPercent:hits/options.spins*100,bonusFrequency:bonuses?options.spins/bonuses:null,bonusFrequencyPercent:bonuses/options.spins*100,averageCascades:cascades/options.spins,maxCascades,averageWin:totalWin/options.spins,medianWin:percentile(payoutFrequency,options.spins,.5),maxWin,maxWinX:maxWin/options.stake,bucketsPercent:Object.fromEntries(Object.entries(ranges).map(([key,value])=>[key,value/options.spins*100])),rtpContributionPercent:{base:baseWin/totalCharged*100,scatter:scatterWin/totalCharged*100,bonus:bonusWin/totalCharged*100},scatterDistribution:record(scatterDistribution),multiplierDistribution:record(multiplierDistribution),averageBonusValue:bonuses?bonusRoundsWin/bonuses:0,averageBonusValueX:bonuses?bonusRoundsWin/bonuses/options.stake:0,percentiles:{p95:percentile(payoutFrequency,options.spins,.95),p99:percentile(payoutFrequency,options.spins,.99),p999:percentile(payoutFrequency,options.spins,.999)},rtpStandardErrorPercent:Math.sqrt(m2/Math.max(1,options.spins-1)/options.spins)*100,config:{baseGameSymbolWeights:LEMONZA_BASE_GAME_SYMBOL_WEIGHTS,boostSymbolWeights:LEMONZA_BOOST_SYMBOL_WEIGHTS,freeSpinsSymbolWeights:LEMONZA_FREE_SPINS_SYMBOL_WEIGHTS,freeSpinsMultiplierChanceBps:LEMONZA_FREE_SPINS_MULTIPLIER_CHANCE_BPS,boostFreeSpinsMultiplierChanceBps:LEMONZA_BOOST_FREE_SPINS_MULTIPLIER_CHANCE_BPS,bonusBuySymbolWeights:LEMONZA_BONUS_BUY_SYMBOL_WEIGHTS,bonusBuyMultiplierChanceBps:LEMONZA_BONUS_BUY_MULTIPLIER_CHANCE_BPS,multiplierValueWeights:LEMONZA_MULTIPLIER_WEIGHTS,paytable:LEMONZA_SYMBOLS}};
}
