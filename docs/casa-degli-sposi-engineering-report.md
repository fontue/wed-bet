...
Director поддерживает `cancel`, `dispose`, pause/resume при visibility, skip текущей фазы и skip до серверного финала. Финальный баланс всегда берётся из `round.balanceAfter`; повтор неизвестного сетевого результата использует тот же idempotency key.
