...
test("dev-сценарии блокируют экономические Spin до fetch",()=>{for(const file of ["../components/slots/dog-house-game.tsx","../components/slots/lemonza-game.tsx"]){const source=readFileSync(new URL(file,import.meta.url),"utf8"),guard=source.indexOf("if(devTest!==null)"),request=source.indexOf('fetch("/api/slots/');assert.ok(guard>=0&&guard<request,file);assert.ok(source.includes("interactionLocked=busy||devTest!==null"),file);}});
test("production dev UI остаётся условным",()=>{for(const file of ["../components/slots/dog-house-game.tsx","../components/slots/lemonza-game.tsx"]){const source=readFileSync(new URL(file,import.meta.url),"utf8");assert.ok(source.includes('const IS_DEVELOPMENT=process.env.NODE_ENV==="development"'),file);assert.ok(source.includes("if(!IS_DEVELOPMENT)return null"),file);}});
test("Casa anticipation может быть показан собственным active-классом",()=>{const css=readFileSync(new URL("../app/slot-polish.css",import.meta.url),"utf8"),game=readFileSync(new URL("../components/slots/dog-house-game.tsx",import.meta.url),"utf8");assert.ok(css.includes(".dogslot-machine .dogslot-anticipation.is-active{display:flex!important"));assert.ok(game.includes('view.state==="confirming-bonus"'));});
